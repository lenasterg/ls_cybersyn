<?php
/*
 * last edit by stergatu on 1/4/2014
 *
...
<div class="wrap">
	...
	if ( isset( $_POST['alter_default_settings'] ) ) {
		echo '<h2> <span class="dashicons dashicons-rss"></span> ' . __( 'RSS/Atom Syndicator - Default Settings', 'cybersyn' ) . '</h2>';
	} else {
		echo '<h1 class="wp-heading-inline">' , __( 'RSS/Atom Syndicator', 'cybersyn' ) . '  <span class="dashicons dashicons-rss"></span> </h1>';
		if ( ! isset( $_GET['edit-feed-id'] ) ) {
			echo $csyn_banner;
		}
		echo '<div><em>Για οδηγίες πατήστε να "ξεδιπλωθεί" η Βοήθεια από την πάνω δεξιά γωνία</em></div>';
	}

	$csyn_syndicator    = new Ls_CyberSyn_Syndicator();
	...
	if ( isset( $_GET['edit-feed-id'] ) ) {
		...
		$csyn_syndicator->feeds [ (int) $_POST['feed_id'] ]['options']['post_status']              = 'pending';
		$csyn_syndicator->feeds [ (int) $_POST['feed_id'] ]['options']['comment_status']           = $_POST['post_comments'];
		$csyn_syndicator->feeds [ (int) $_POST['feed_id'] ]['options']['ping_status']              = 'closed';
		$csyn_syndicator->feeds [ (int) $_POST['feed_id'] ]['options']['post_author']              = intval( $_POST['post_author'] );
		$csyn_syndicator->feeds [ (int) $_POST['feed_id'] ]['options']['base_date']                = 'syndication';
		...
		$feed['options']['ping_status']              = 'closed';
		$feed['options']['post_author']              = intval( $_POST['post_author'] );
		$feed['options']['base_date']                = 'syndication';
		...
		$csyn_syndicator->global_options ['ping_status']              = 'closed';
