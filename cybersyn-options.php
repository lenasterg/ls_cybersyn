...
if ( isset( $_POST['Submit'] ) ) {
	...
	$update_csyn_text[]    = __( 'RSS pull mode', 'cybersyn' );
	$update_csyn_text[]    = __( 'Pseudo cron interval', 'cybersyn' );
	$update_csyn_text[]    = __( 'Feed duplication control', 'cybernet' );
	$update_csyn_text[]    = __( 'Link to source' );
	$i                     = 0;
	$text                  = '';
	foreach ( $update_csyn_queries as $update_csyn_query ) {
		if ( $update_csyn_query ) {
			$text .= $update_csyn_text[ $i ] . ' ' . __( 'Updated', 'cybersyn' ) . '<br />';
			if ( 'RSS pull mode' === $update_csyn_text[ $i ] || 'Pseudo cron interval' === $update_csyn_text[ $i ] ) {
	...
	if ( empty( $text ) ) {
		$text = __( 'No Option Updated', 'cybersyn' );
	}
}
...
<div class="wrap">
	...
	<h2><?php _e( 'General Settings', 'cybersyn' ); ?></h2>
	...
	<form method="post" action="<?php echo csyn_REQUEST_URI(); ?>" name="general_settings">
		...
		<table class="form-table">
			<tr valign="top">
				...
				<td align="left">
					<select style="width: 160px;" name="<?php echo CSYN_RSS_PULL_MODE; ?>" onchange="changeMode();"
																   <?php
	 ...
	 size="1">
	 <?php
		echo '<option ' . ( ( 'auto' === get_option( CSYN_RSS_PULL_MODE ) ) ? 'selected ' : '' ) . 'value="auto">auto</option>';
		echo '<option ' . ( ( 'cron' === get_option( CSYN_RSS_PULL_MODE ) ) ? 'selected ' : '' ) . 'value="cron">by cron job or manually</option>';
		?>
