...
/*
  ...
  Version: 4
  Author: CyberSEO.net and Stergatu Lena
...
$csyn_banner = '<div id="welcome-panel" class="' . esc_attr( $classes ) . '"><div class="welcome-panel-content">' .
//wp_nonce_field( 'welcome-panel-nonce' , 'welcomepanelnonce' , false ) .
        ls_cybersyn_help_overview_content() .
//        '
//
//                    <div class="welcome-panel-column-container-lssyn">
//                        <div class="welcome-panel-column-lssyn">
//                           ' . ls_cybersyn_help_add_content() . '
//                        </div>
//                        <div class="welcome-panel-column-last-lssyn">
//                          ' . ls_cybersyn_help_existed_content() .
//                        '</div>
//                    </div>
//                </div>'.
//
 ...
 * @param type $options
 * @return int
 * @version 2, stergatu change default
 */
...
//                                echo ' <a href=edit.php?post_status=pending&post_type=post>' . __( 'Go to pending posts' , 'cybersyn' ) . '</a>' ;
     ...
     * @version 2, 1/7/2014, stergatu add log
     *
     */
...
//                                }
        ...
        <!--                <tr>
                    <td><?php _e( 'Posts status' , 'cybersyn' ) ; ?></td>
...
//                            echo '<option ' . (($settings[ "post_status" ] == "private") ? 'selected ' : '') . 'value="private">Save as private</option>' . "\n" ;
                ...
                <tr>
                    ...
                    <td><select name="post_comments" size="1">
                            <?php
                            echo '<option ' . (($settings[ 'comment_status' ] == 'open') ? 'selected ' : '') . 'value="open">' ;
                ...
                ?>
                <?php _e( '- here you can set the syndication date adjustment range in minutes. This range will be used to randomly adjust the publication date for every aggregated post. For example, if you set
                                the adjustment range as [0..60], the post dates will be increased by a random value between 0 and 60 minutes.' , 'cybersyn' ) ; ?>
                                                                                                                                                                                                                                                                                                                                                                                                                                    </td>
                                                                                                                                                                                                                                                                                                                        </tr>-->
                <tr>
    ...
    function showMainPage( $showsettings = true ) {
        ...
        echo "<tr><td align=\"left\"><h3>\n" ;
        echo __( 'New Feed URL' , 'cybersyn' ) . ' <input type="text" name="feed_url" value="" size="100">' . "\n" ;
...
/**
 *
 * @global CyberSyn_Syndicator $csyn_syndicator
 * @param type $content
 * @return type
 * @version 2, stergatu
 */
...
function csyn_main_menu() {
    global $ls_cybersyn_plugin_hook ;

    if ( function_exists( 'add_posts_page' ) ) {

//        add_submenu_page( '/edit.php' , 'Άντληση άρθρων από εξωτερικές πηγές' , 'Άντληση άρθρων από εξωτερικές πηγές' , 'add_users' , DIRNAME( __FILE__ ) . '/cybersyn-syndicator.php' ) ;
        $ls_cybersyn_plugin_hook = add_posts_page( 'Άντληση άρθρων από εξωτερικές πηγές' , 'Άντληση άρθρων από εξωτερικές πηγές' , 'add_users' , DIRNAME( __FILE__ ) . '/cybersyn-syndicator.php' ) ;
    }
}

function ls_cybersyn_help( $screen = '' ) {
...
// help tabs
        ...
        $screen->add_help_tab( array (
            'id' => 'ls_cybersyn_add' ,
            'title' => __( 'Ορισμός νέας εξωτερικής πηγής' ) ,
...
add_filter( 'contextual_help' , 'ls_cybersyn_help' ) ;
...
function ls_cybersyn_admin_enqueue() {
    ...
    wp_register_script(
        'ls_cybersyn_admin_js', // Script handle
        plugins_url( '/js/masonry.pkgd.min.js' , __FILE__ ) , // Script URL
    ...
    wp_enqueue_script( 'ls_cybersyn_admin_js' ) ;
    //wp_enqueue_script( 'ls_cybersyn_admin_classie_js' ) ;
}
...
function ls_cybersyn_log( $feed , $deleted = null ) {
    ...
    if ( $deleted ) {
        $wpdb->query( $wpdb->prepare( "UPDATE  $table SET deleted =  1   WHERE blog_id=%d AND feed_url=%s" , array (
                    $blog_id ,
                    $feed[ 'url' ] ,
                        )
        ) ) ;
	...
	" , array (
            ...
            $wpdb->query( $wpdb->prepare( "UPDATE  $table SET last_updated =  now() , deleted=NULL   WHERE blog_id=%d AND feed_url=%s" , array (
                        $blog_id ,
                        $feed[ 'url' ] ,
                            )
            ) ) ;
...
function ls_admin_posts_filter( $query ) {
    global $pagenow ;
    if ( is_admin() && $pagenow == 'edit.php' && isset( $_GET[ 'ls_source' ] ) && $_GET[ 'ls_source' ] != '' ) {
...
function ls_admin_posts_filter_restrict_manage_posts() {
    ...
    <select name="ls_source">
        ...
        foreach ( $fields as $field ) {
            if ( substr( $field[ 0 ] , 0 , 1 ) != "_" ) {
                printf
                        (
                        '<option value="%s"%s>%s</option>' , $field[ 0 ] , $field[ 0 ] == $current ? ' selected="selected"' : '' , $field[ 0 ]
                ) ;
            }
        }
        ?>
    ...
    <?php // _e( 'Value:' , 'baapf' ) ;   ?>
    <!--<input type="TEXT" name="ls_source" value="<?php echo $current_v ; ?>" />-->
    <?php
}
