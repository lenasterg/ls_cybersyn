...
';
	...
	function ls_cybersyn_help() {
		...
		if ( 'ls_cybersyn/cybersyn-syndicator' === $screen->id ) {
			// help tabs
			$screen->add_help_tab(
				array(
	...
	function csyn_generic_ping( $post_id ) {
		...
		$dates = $wpdb->get_row( $wpdb->prepare( "SELECT post_date, post_modified FROM $wpdb->posts WHERE id=%d", $post_id ) );
		if ( ! is_null( $csyn_syndicator ) ) {
	...
	if ( 'on' === get_option( CSYN_LINK_TO_SOURCE ) ) {
